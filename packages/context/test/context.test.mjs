...
test('MarkdownRenderer.getBlockTier classifies blocks into 5 architectural tiers', () => {
  ...
  const t6Dynamic = MarkdownRenderer.getBlockTier({ id: 'block-unassigned-custom', title: 'Dynamic Working Set' });
  assert.equal(t6Dynamic, 'Tier 6: Workspace & Dynamic Blocks');
});
...
test('MarkdownRenderer handles edge cases: null refs, string refs, and unassigned dynamic blocks', () => {
  // 1. Dynamic block with mcp or application files must stay in Tier 6
  const t6Dynamic = MarkdownRenderer.getBlockTier({
    id: 'block-unassigned-task-999',
    title: 'Adaptive Working Set',
    artifactRefs: ['packages/mcp/src/v2-service.mjs', 'packages/application/src/task-service.mjs'],
  });
  assert.equal(t6Dynamic, 'Tier 6: Workspace & Dynamic Blocks');

  // 2. Null/undefined elements in artifactRefs must not crash getBlockTier
  ...
  // 3. String artifactRefs in renderBlock must render cleanly without 'undefined'
